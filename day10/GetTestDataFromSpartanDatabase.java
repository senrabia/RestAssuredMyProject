package day10;
//USER NAME , PASS WORD MUTLAKA GEREKLI DATAYA GIRMEK ICIN.
//CDBC KODLAININ TAMAMINDA TRY CATCH KULLANILDI.
...
public class GetTestDataFromSpartanDatabase {
    ...
    public void testDataFromDB(){
        ...
        Map<String, String> firstRowMap = DB_Utility.getRowMap(1);//1 RAW DAKI SONUCU ALDIM
        // get the id , name , gender , phone out of this map
        System.out.println("firstRowMap = " + firstRowMap);
        System.out.println("SPARTAN_ID key's value = " + firstRowMap.get("SPARTAN_ID")  );//SPARTAN_ID YERINE NAME YAZSAK TA SONUCU VERIR
        ...
        int idFromDB        = Integer.parseInt( firstRowMap.get("SPARTAN_ID") );// BURDA WAPPER CLASS KULLANDIK
        String nameFromDB   = firstRowMap.get("NAME") ;
        String genderFromDB = firstRowMap.get("GENDER") ;
        long phoneFromDB    = Long.parseLong(firstRowMap.get("PHONE"));//PARSE METODU WAPPER CLASS

        when()
                .get("/spartans/{id}" , idFromDB ). //"/spartans/{id}" BURASI BENIM END POINTIM
                then()
                .log().all()
                .statusCode(200)
                .body("id",  is(idFromDB) )
                ...
                .body("phone.toLong()", is(phoneFromDB) ) ;//INTEGER I LONG A CEVIRDIK BURADA
    ...
    public void testDataFromDB_RandomLy(){
        ...
        int randomRowNum = new Faker().number().numberBetween(1, rowCount) ;//BURASI FAKER DAN GELIYOR
